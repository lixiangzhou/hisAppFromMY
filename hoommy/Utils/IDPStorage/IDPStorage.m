...
static dispatch_queue_t get_idp_diskio_queue()
{
    ...
    dispatch_once(&onceToken, ^{
        idpDiskIOQueue = dispatch_queue_create("com.baidu.idp", DISPATCH_QUEUE_SERIAL);
    });
...
{
    IDPLogDebug(@"initWithNameSpace with type %d",type);
    self = [super init];
    if(self)
    {
...
{
    IDPLogTrace(@"dataForKey key = %@",key);
...
{
    IDPLogDebug(@"dataForKeyAsy key = %@",key);
    dispatch_async(get_idp_diskio_queue(), ^{
        NSError* error = nil;
        NSData* data = [self.storageInnerEngine  loadObjectForKey:key error:&error];
...
{
    IDPLogDebug(@"setDataAsy key = %@",key);
    dispatch_async(get_idp_diskio_queue(), ^{
        NSError* error = nil;
        BOOL bSuccess  = [self.storageInnerEngine saveObject:obj forKey:key error:&error];
        if (completionHandler)
        {
            dispatch_async(dispatch_get_main_queue(),^{
...
{
    IDPLogDebug(@"removeObjectForKeyAsy key = %@",key);
    dispatch_async(get_idp_diskio_queue(), ^{
        NSError* error = nil;
        BOOL bSuccess  = [self.storageInnerEngine removeObjectForKey:key error:&error];
        if (completionHandler) {
            dispatch_async(dispatch_get_main_queue(),^{
                completionHandler(bSuccess,error);
            });
...
{
    IDPLogDebug(@"cleanNameSpace %@",nameSpace);
    if(type == IDPStorageDisk)
    {
    ...
    return NO;
    IDPLogDebug(@"cleanNameSpace %@ end",nameSpace);
}
...
+(void)cleanNameSpace:(NSString*)nameSpace type:(IDPStorageType)type completionHandle:(void(^)(BOOL success,NSError* error))completionHandler
{
    dispatch_async(get_idp_diskio_queue(), ^{
        ...
        if(type == IDPStorageDisk)
        {
            bSuccess = [IDPStorageFileInner cleanNameSpace:nameSpace error:&error];
        }
        else if(type == IDPStorageSql)
        {
            bSuccess = [IDPStorageSqliteInner cleanNameSpace:nameSpace error:&error];
        }
        if (completionHandler) {
            dispatch_async(dispatch_get_main_queue(),^{
                completionHandler(bSuccess,error);
...
+ (long long)getSpaceDiskSize:(NSString *)nameSpace
{
    if (nil == nameSpace) {
        return 0;
    }
    return [IDPStorageFileInner getNameSpaceSize:nameSpace];
}


- (id)objectForKey:(NSString *)key error:(NSError **)error
{
    IDPLogTrace(@"objectForKey key = %@",key);
    //保证线程安全使用sync和async调用一个队列
    __block NSData* data = nil;
    __block id obj = nil;
    dispatch_sync(get_idp_diskio_queue(), ^{
...
- (BOOL)setObject:(NSData *)data forKey:(NSString *)key type:(IDPCacheObjectType)type error:(NSError **)error
{
    ...
    dispatch_sync(get_idp_diskio_queue(), ^{
        ...
        success = [self setData:saveData forKey:key error:nil];
    });
    return success;
}
...
- (void)setObject:(NSData*)data forKey:(NSString*)key type:(IDPCacheObjectType)type completionHandle:(void(^)(BOOL success,NSError* error))completionHandler
{
    dispatch_async(get_idp_diskio_queue(), ^{
        NSMutableData *saveData = [[NSMutableData alloc] initWithBytes:&type length:sizeof(type)];
        [saveData appendData:data];
        return [self setData:saveData forKey:key completionHandle:completionHandler];
//        [saveData release];
    });
}
