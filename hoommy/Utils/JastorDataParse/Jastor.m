...
- (void)setDataFromDictionary:(NSDictionary *)dictionary{
    for (NSString *key in [JastorRuntimeHelper propertyNames:[self class]]) {
        ...
        // handle all others
        [self setFiledValue:value propertyName:key];
    }
...
- (void)setFiledValue:(id)value propertyName:(NSString*)key {
    NSString* typeName = [JastorRuntimeHelper propertyTypeName:[self class] propertyName:key];
    if(typeName && ![[value class] isSubclassOfClass:NSClassFromString(typeName)]) {
